...
class GuiControllerTests(unittest.TestCase):
    ...
    def test_startup_backup_creates_dated_timed_snapshots_and_prunes(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            second = create_startup_backup(
                backup_dir=backups, export_dir=exports, state_file=state,
                config_file=config, roster_file=roster, keep=2,
                snapshot_at=datetime(2026, 8, 20, 10, 45, 5))
            self.assertEqual(second.name, "10-45-05")
            result.write_text('{"value":2}')
            create_startup_backup(backup_dir=backups, export_dir=exports, state_file=state,
                                  config_file=config, roster_file=roster, keep=2,
                                  snapshot_at=datetime(2026, 8, 21, 8, 0, 0))
            snapshots = [path for day in backups.iterdir() if day.is_dir()
                         for path in day.iterdir() if path.is_dir()]
            self.assertEqual(sorted((path.parent.name, path.name) for path in snapshots),
                             [("08-20-2026", "10-45-05"),
                              ("08-21-2026", "08-00-00")])
